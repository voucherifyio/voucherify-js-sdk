/**
 * Voucherify API
 * Voucherify promotion engine REST API. Please see https://docs.voucherify.io/docs for more details.
 *
 * OpenAPI spec version: v2018-08-01
 * Contact: support@voucherify.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { LoyaltyCardTransaction } from '../models/LoyaltyCardTransaction';
import { HttpFile } from '../http/http';

/**
* Response body schema for **GET** `v1/loyalties/{campaignId}/members/{memberId}/transactions` and `/loyalties/members/{memberId}/transactions`.
*/
export class LoyaltiesMembersTransactionsListResponseBody {
    /**
    * The type of the object represented by JSON.
    */
    'object'?: LoyaltiesMembersTransactionsListResponseBodyObjectEnum | null;
    /**
    * Identifies the name of the attribute that contains the array of transaction objects.
    */
    'dataRef'?: LoyaltiesMembersTransactionsListResponseBodyDataRefEnum | null;
    /**
    * A dictionary that contains an array of transactions. Each entry in the array is a separate transaction object.
    */
    'data'?: Array<LoyaltyCardTransaction> | null;
    /**
    * As query results are always limited (by the limit parameter), the `has_more` flag indicates if there are more records for given filter parameters. This lets you know if you can run another request to get more records returned in the results.
    */
    'hasMore'?: boolean | null;
    /**
    * Returns an ID that can be used to return another page of results. Use the transaction ID in the `starting_after_id` query parameter to display another page of the results starting after the transaction with that ID.
    */
    'moreStartingAfter'?: string | null;

    static readonly discriminator: string | undefined = undefined;

    static readonly mapping: {[index: string]: string} | undefined = undefined;

    static readonly attributeTypeMap: Array<{name: string, baseName: string, type: string, format: string}> = [
        {
            "name": "object",
            "baseName": "object",
            "type": "LoyaltiesMembersTransactionsListResponseBodyObjectEnum",
            "format": ""
        },
        {
            "name": "dataRef",
            "baseName": "data_ref",
            "type": "LoyaltiesMembersTransactionsListResponseBodyDataRefEnum",
            "format": ""
        },
        {
            "name": "data",
            "baseName": "data",
            "type": "Array<LoyaltyCardTransaction>",
            "format": ""
        },
        {
            "name": "hasMore",
            "baseName": "has_more",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "moreStartingAfter",
            "baseName": "more_starting_after",
            "type": "string",
            "format": ""
        }    ];

    static getAttributeTypeMap() {
        return LoyaltiesMembersTransactionsListResponseBody.attributeTypeMap;
    }

    public constructor() {
    }
}

export enum LoyaltiesMembersTransactionsListResponseBodyObjectEnum {
    List = 'list'
}
export enum LoyaltiesMembersTransactionsListResponseBodyDataRefEnum {
    Data = 'data'
}

