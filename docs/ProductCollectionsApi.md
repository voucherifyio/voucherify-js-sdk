...
> ProductCollectionsCreateResponseBody createProductCollection(opts)
...
let opts = {
  'productCollectionsCreateRequestBody': new Voucherify.ProductCollectionsCreateRequestBody() // ProductCollectionsCreateRequestBody | 
};
apiInstance.createProductCollection(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
