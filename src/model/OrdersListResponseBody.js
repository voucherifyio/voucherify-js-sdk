/**
 * Voucherify API
 * Voucherify promotion engine REST API. Please see https://docs.voucherify.io/docs for more details.
 *
 * The version of the OpenAPI document: v2018-08-01
 * Contact: support@voucherify.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import OrdersListResponseBodyOrdersItem from './OrdersListResponseBodyOrdersItem';

/**
 * The OrdersListResponseBody model module.
 * @module model/OrdersListResponseBody
 * @version v2018-08-01
 */
class OrdersListResponseBody {
    /**
     * Constructs a new <code>OrdersListResponseBody</code>.
     * Response body schema representing **GET** &#x60;v1/orders&#x60;.
     * @alias module:model/OrdersListResponseBody
     */
    constructor() { 
        
        OrdersListResponseBody.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
        obj['object'] = 'list';
        obj['data_ref'] = 'orders';
    }

    /**
     * Constructs a <code>OrdersListResponseBody</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/OrdersListResponseBody} obj Optional instance to populate.
     * @return {module:model/OrdersListResponseBody} The populated <code>OrdersListResponseBody</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new OrdersListResponseBody();

            if (data.hasOwnProperty('object')) {
                obj['object'] = ApiClient.convertToType(data['object'], 'String');
            }
            if (data.hasOwnProperty('data_ref')) {
                obj['data_ref'] = ApiClient.convertToType(data['data_ref'], 'String');
            }
            if (data.hasOwnProperty('orders')) {
                obj['orders'] = ApiClient.convertToType(data['orders'], [OrdersListResponseBodyOrdersItem]);
            }
            if (data.hasOwnProperty('total')) {
                obj['total'] = ApiClient.convertToType(data['total'], 'Number');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>OrdersListResponseBody</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>OrdersListResponseBody</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['object'] && !(typeof data['object'] === 'string' || data['object'] instanceof String)) {
            throw new Error("Expected the field `object` to be a primitive type in the JSON string but got " + data['object']);
        }
        // ensure the json data is a string
        if (data['data_ref'] && !(typeof data['data_ref'] === 'string' || data['data_ref'] instanceof String)) {
            throw new Error("Expected the field `data_ref` to be a primitive type in the JSON string but got " + data['data_ref']);
        }
        if (data['orders']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['orders'])) {
                throw new Error("Expected the field `orders` to be an array in the JSON data but got " + data['orders']);
            }
            // validate the optional field `orders` (array)
            for (const item of data['orders']) {
                OrdersListResponseBodyOrdersItem.validateJSON(item);
            };
        }

        return true;
    }


}



/**
 * The type of the object represented by JSON. This object stores information about orders in a dictionary.
 * @member {module:model/OrdersListResponseBody.ObjectEnum} object
 * @default 'list'
 */
OrdersListResponseBody.prototype['object'] = 'list';

/**
 * Identifies the name of the attribute that contains the array of order objects.
 * @member {module:model/OrdersListResponseBody.DataRefEnum} data_ref
 * @default 'orders'
 */
OrdersListResponseBody.prototype['data_ref'] = 'orders';

/**
 * Contains array of order objects.
 * @member {Array.<module:model/OrdersListResponseBodyOrdersItem>} orders
 */
OrdersListResponseBody.prototype['orders'] = undefined;

/**
 * Total number of orders.
 * @member {Number} total
 */
OrdersListResponseBody.prototype['total'] = undefined;





/**
 * Allowed values for the <code>object</code> property.
 * @enum {String}
 * @readonly
 */
OrdersListResponseBody['ObjectEnum'] = {

    /**
     * value: "list"
     * @const
     */
    "list": "list"
};


/**
 * Allowed values for the <code>data_ref</code> property.
 * @enum {String}
 * @readonly
 */
OrdersListResponseBody['DataRefEnum'] = {

    /**
     * value: "orders"
     * @const
     */
    "orders": "orders"
};



export default OrdersListResponseBody;

