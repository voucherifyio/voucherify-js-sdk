/**
 * Voucherify API
 * Voucherify promotion engine REST API. Please see https://docs.voucherify.io/docs for more details.
 *
 * The version of the OpenAPI document: v2018-08-01
 * Contact: support@voucherify.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import LoyaltiesRewardsGetResponseBodyParametersLoyalty from './LoyaltiesRewardsGetResponseBodyParametersLoyalty';

/**
 * The LoyaltiesRewardsGetResponseBodyParameters model module.
 * @module model/LoyaltiesRewardsGetResponseBodyParameters
 * @version v2018-08-01
 */
class LoyaltiesRewardsGetResponseBodyParameters {
    /**
     * Constructs a new <code>LoyaltiesRewardsGetResponseBodyParameters</code>.
     * Defines the cost of the reward.
     * @alias module:model/LoyaltiesRewardsGetResponseBodyParameters
     */
    constructor() { 
        
        LoyaltiesRewardsGetResponseBodyParameters.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>LoyaltiesRewardsGetResponseBodyParameters</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/LoyaltiesRewardsGetResponseBodyParameters} obj Optional instance to populate.
     * @return {module:model/LoyaltiesRewardsGetResponseBodyParameters} The populated <code>LoyaltiesRewardsGetResponseBodyParameters</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new LoyaltiesRewardsGetResponseBodyParameters();

            if (data.hasOwnProperty('loyalty')) {
                obj['loyalty'] = LoyaltiesRewardsGetResponseBodyParametersLoyalty.constructFromObject(data['loyalty']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>LoyaltiesRewardsGetResponseBodyParameters</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>LoyaltiesRewardsGetResponseBodyParameters</code>.
     */
    static validateJSON(data) {
        // validate the optional field `loyalty`
        if (data['loyalty']) { // data not null
          LoyaltiesRewardsGetResponseBodyParametersLoyalty.validateJSON(data['loyalty']);
        }

        return true;
    }


}



/**
 * @member {module:model/LoyaltiesRewardsGetResponseBodyParametersLoyalty} loyalty
 */
LoyaltiesRewardsGetResponseBodyParameters.prototype['loyalty'] = undefined;






export default LoyaltiesRewardsGetResponseBodyParameters;

