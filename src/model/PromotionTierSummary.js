/**
 * Voucherify API
 * Voucherify promotion engine REST API. Please see https://docs.voucherify.io/docs for more details.
 *
 * The version of the OpenAPI document: v2018-08-01
 * Contact: support@voucherify.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import PromotionTierSummaryOrders from './PromotionTierSummaryOrders';
import PromotionTierSummaryRedemptions from './PromotionTierSummaryRedemptions';

/**
 * The PromotionTierSummary model module.
 * @module model/PromotionTierSummary
 * @version v2018-08-01
 */
class PromotionTierSummary {
    /**
     * Constructs a new <code>PromotionTierSummary</code>.
     * Contains statistics about promotion tier redemptions and orders.
     * @alias module:model/PromotionTierSummary
     */
    constructor() { 
        
        PromotionTierSummary.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>PromotionTierSummary</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/PromotionTierSummary} obj Optional instance to populate.
     * @return {module:model/PromotionTierSummary} The populated <code>PromotionTierSummary</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new PromotionTierSummary();

            if (data.hasOwnProperty('redemptions')) {
                obj['redemptions'] = PromotionTierSummaryRedemptions.constructFromObject(data['redemptions']);
            }
            if (data.hasOwnProperty('orders')) {
                obj['orders'] = PromotionTierSummaryOrders.constructFromObject(data['orders']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>PromotionTierSummary</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>PromotionTierSummary</code>.
     */
    static validateJSON(data) {
        // validate the optional field `redemptions`
        if (data['redemptions']) { // data not null
          PromotionTierSummaryRedemptions.validateJSON(data['redemptions']);
        }
        // validate the optional field `orders`
        if (data['orders']) { // data not null
          PromotionTierSummaryOrders.validateJSON(data['orders']);
        }

        return true;
    }


}



/**
 * @member {module:model/PromotionTierSummaryRedemptions} redemptions
 */
PromotionTierSummary.prototype['redemptions'] = undefined;

/**
 * @member {module:model/PromotionTierSummaryOrders} orders
 */
PromotionTierSummary.prototype['orders'] = undefined;






export default PromotionTierSummary;

