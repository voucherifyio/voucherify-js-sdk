/**
 * Voucherify API
 * Voucherify promotion engine REST API. Please see https://docs.voucherify.io/docs for more details.
 *
 * The version of the OpenAPI document: v2018-08-01
 * Contact: support@voucherify.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Bundle from './Bundle';
import Discount from './Discount';
import ModelError from './ModelError';
import RedeemableGift from './RedeemableGift';
import RedeemableLoyaltyCard from './RedeemableLoyaltyCard';

/**
 * The RedeemableResult model module.
 * @module model/RedeemableResult
 * @version v2018-08-01
 */
class RedeemableResult {
    /**
     * Constructs a new <code>RedeemableResult</code>.
     * Information about redeemable result.
     * @alias module:model/RedeemableResult
     */
    constructor() { 
        
        RedeemableResult.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>RedeemableResult</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RedeemableResult} obj Optional instance to populate.
     * @return {module:model/RedeemableResult} The populated <code>RedeemableResult</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new RedeemableResult();

            if (data.hasOwnProperty('discount')) {
                obj['discount'] = Discount.constructFromObject(data['discount']);
            }
            if (data.hasOwnProperty('bundle')) {
                obj['bundle'] = Bundle.constructFromObject(data['bundle']);
            }
            if (data.hasOwnProperty('gift')) {
                obj['gift'] = RedeemableGift.constructFromObject(data['gift']);
            }
            if (data.hasOwnProperty('loyalty_card')) {
                obj['loyalty_card'] = RedeemableLoyaltyCard.constructFromObject(data['loyalty_card']);
            }
            if (data.hasOwnProperty('error')) {
                obj['error'] = ModelError.constructFromObject(data['error']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>RedeemableResult</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>RedeemableResult</code>.
     */
    static validateJSON(data) {
        // validate the optional field `discount`
        if (data['discount']) { // data not null
          Discount.validateJSON(data['discount']);
        }
        // validate the optional field `bundle`
        if (data['bundle']) { // data not null
          Bundle.validateJSON(data['bundle']);
        }
        // validate the optional field `gift`
        if (data['gift']) { // data not null
          RedeemableGift.validateJSON(data['gift']);
        }
        // validate the optional field `loyalty_card`
        if (data['loyalty_card']) { // data not null
          RedeemableLoyaltyCard.validateJSON(data['loyalty_card']);
        }
        // validate the optional field `error`
        if (data['error']) { // data not null
          ModelError.validateJSON(data['error']);
        }

        return true;
    }


}



/**
 * @member {module:model/Discount} discount
 */
RedeemableResult.prototype['discount'] = undefined;

/**
 * @member {module:model/Bundle} bundle
 */
RedeemableResult.prototype['bundle'] = undefined;

/**
 * @member {module:model/RedeemableGift} gift
 */
RedeemableResult.prototype['gift'] = undefined;

/**
 * @member {module:model/RedeemableLoyaltyCard} loyalty_card
 */
RedeemableResult.prototype['loyalty_card'] = undefined;

/**
 * @member {module:model/ModelError} error
 */
RedeemableResult.prototype['error'] = undefined;






export default RedeemableResult;

